    ...
    <div className="min-h-screen bg-gradient-to-b from-yellow-50 to-amber-50">
      <div className="max-w-md mx-auto px-4 py-6">
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-yellow-800">记录大便</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {POOP_SHAPES.map((s) => (
                <label
                  ...
                  className={`flex items-center p-3 rounded-lg border-2 cursor-pointer transition-colors ${
                    shape === s ? 'border-yellow-400 bg-yellow-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
            ...
            <div className="space-y-2">
              {POOP_COLORS.map((c) => (
                <label
                  key={c}
                  className={`flex items-center p-3 rounded-lg border-2 cursor-pointer transition-colors ${
                    color === c ? 'border-yellow-400 bg-yellow-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
          ...
          <div className="bg-white rounded-xl p-5 shadow-sm">
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none"
            />
          </div>

          <button
            type="submit"
            className="w-full bg-gradient-to-r from-yellow-400 to-yellow-500 text-white py-4 rounded-xl font-medium text-lg shadow-md active:scale-95 transition-transform"
          >
            保存记录
          </button>
        </form>
